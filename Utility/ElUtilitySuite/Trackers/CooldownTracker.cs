...
namespace ElUtilitySuite.Trackers
{
    ...
    internal class CoolDownTimers : IPlugin
    {
        ...
        {
            return abilityTracker[item].Cast<CheckBox>().CurrentValue;
        }

        public static int getSliderItem(string item)
        {
            return abilityTracker[item].Cast<Slider>().CurrentValue;
        }
        #endregion
       ...
       public static Menu rootMenu = ElUtilitySuite.Entry.menu;
        public static Menu abilityTracker;
        public void CreateMenu(Menu rootMenu)
        {
            abilityTracker = rootMenu.AddSubMenu("Ability tracker", "cdtracker");
            abilityTracker.Add("OffsetTop", new Slider("Offset Top", 0, 0, 0));
            abilityTracker.Add("OffsetLeft", new Slider("Offset Left", 0, 0, 0));
            abilityTracker.Add("Outline", new CheckBox("Outline"));
            abilityTracker.Add("Self", new CheckBox("Self", false));
            abilityTracker.Add("Enemy", new CheckBox("Enemy", false));
            abilityTracker.Add("Ally", new CheckBox("Ally", false));
            abilityTracker.Add("Enabled", new CheckBox("Enabled"));
        ...
        private void OnDrawingEndScene(EventArgs args)
        {
            try
            {
                ...
                var outline = getCheckBoxItem("Outline");
                var offsetTop = getSliderItem("OffsetTop");
                var offsetLeft = getSliderItem("OffsetLeft"); 
        ...
        private void OnGameObjectCreate(GameObject sender, EventArgs args)
        {
            try
            {
                if (!sender.IsValid || sender.Type != GameObjectType.obj_GeneralParticleEmitter
                    || sender.IsMe && !getCheckBoxItem("Self")
                    || sender.IsAlly && !getCheckBoxItem("Ally")
                    || sender.IsEnemy && !getCheckBoxItem("Enemy"))
                {
                    return;
                }
